...
export class DataTable implements OnChanges, DoCheck {
  ...
  @Input('afData') public inputData: any[] = [];

  @Input('afSortBy') public sortBy: string | string[] = '';
  @Input('afSortOrder') public sortOrder = 'asc';
  @Output('afSortByChange') public sortByChange = new EventEmitter<
    string | string[]
  >();
  @Output('afSortOrderChange') public sortOrderChange = new EventEmitter<
    string
  >();

  @Input('afRowsOnPage') public rowsOnPage = 1000;
  @Input('afActivePage') public activePage = 1;
  ...
  public ngOnChanges(changes: { [key: string]: SimpleChange }): any {
    ...
    if (changes['sortBy'] || changes['sortOrder']) {
      if (!_.includes(['asc', 'desc'], this.sortOrder)) {
        console.warn(
          "datatable: value for input afSortOrder must be one of ['asc', 'desc'], but is:",
          this.sortOrder
        );
        this.sortOrder = 'asc';
      }
